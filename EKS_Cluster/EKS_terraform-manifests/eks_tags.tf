...
resource "aws_ec2_tag" "eks_subnet_tag_public_elb" {
  #for_each    = toset(data.terraform_remote_state.vpc.outputs.aws_subnet_public_id)
  for_each = toset([data.terraform_remote_state.vpc.outputs.aws_subnet_public_id])
...
resource "aws_ec2_tag" "eks_subnet_tag_private_elb" {
  for_each    = toset([data.terraform_remote_state.vpc.outputs.aws_subnet_private_id])
  resource_id = each.value
